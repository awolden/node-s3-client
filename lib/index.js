  ...
  function checkDoMoreWork() {
    ...
    function downloadS3Object() {
      ...
      function startDownload() {
        ...
        var fullKey = path.basename(s3Object.Key);
        var fullBucket = path.join(bucket, path.dirname(s3Object.Key));
        upDownFileParams.s3Params.Key = fullKey;
        upDownFileParams.s3Params.Bucket = fullBucket;
    ...
    function uploadLocalFile() {
      ...
      function startUpload() {
        ...
        var fullKey = path.basename(localFileStat.s3Path);
        var filePath = path.dirname(localFileStat.s3Path);
        var fullBucket = path.join(upDownFileParams.s3Params.Bucket, params.s3Params.Prefix, filePath);
        upDownFileParams.s3Params.Key = fullKey;
        upDownFileParams.s3Params.Bucket = fullBucket;
        upDownFileParams.localFile = fullPath;
